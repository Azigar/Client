...
/**
 * Created by Azigar on 04.08.2015.
 */
public class NewGoogle extends Thread {
    ...
    public NewGoogle(Handler h1, SocketConfig con) {  //главный
        this.h = h1;
        this.conf = con;
            ...
            try {
                ...
                if (cmd.equalsIgnoreCase(null)) { //если нет сообщения от сервера
                    ...
                    if (cmd.equalsIgnoreCase("YES_HERO")) {
                        conf.setSOCKET_MESSAGE("NEW_GOOGLE");
                    }
                    //сервер оповещает об ошибке запроса
                    if (cmd.equalsIgnoreCase("ERROR")) {
                        h.sendEmptyMessage(3);
                    }
                    //сервер спрашивает Новое имя
                    if (cmd.equalsIgnoreCase("NEW_GOOGLE")) {
                        h.sendEmptyMessage(13);
                    }
                    //сервер изменил пароль
                    if (cmd.equalsIgnoreCase("YES_CHANGE")) {
                        h.sendEmptyMessage(15);
                    }
                    //сервер спрашивает Новое имя
                    if (cmd.equalsIgnoreCase("GOOGLE_EXISTS")) {
